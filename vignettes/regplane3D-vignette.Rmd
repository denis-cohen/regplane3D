...
The example below illustrates the functionality. The function takes extends the
...
range be split into 7 equally spaced intervals. This is reject by the function,
as such a division would not yield pretty values. Instead, the function returns
`nlines = 6`, which means that the lines should be drawn at -4, -2, 0, 2, 4, and
6. However, for finer predictions, we will not compute expected values as these
coarse values, but (per `multiply = 4`) at gradations that divide each interval
into four steps of 0.5.
...
```{r use-plane3D-step4}
## Heatmap values
growth_cat <- cut(us$growth, breaks = growth_axis$seq)
approval_cat <- cut(us$approval, breaks = approval_axis$seq)
