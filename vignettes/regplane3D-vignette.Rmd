...
vignette: >
  %\VignetteIndexEntry{regplane3D-vignette}
...
The `regplane3D` package is a convenience wrapper to Karline Soetaert's
...
For instance, the package can plot conditional expected values of an outcome
variable $Z$ over the joint distribution of two continuous predictors,
$X$ and $Y$, i.e., $\mathbb{E}(Z|X,Y)$.

`regplane3D` (Version 0.1.0) consists of three functions:

1. `regplane3D`: Plot a three-dimensional regression prediction with confidence intervals.
1. `twoplanes3D`: Plot a three-dimensional regression prediction with two planes, typically separated at a cut point in one of the two horizontal dimensions, and their respective confidence intervals.
1. `heatmap3D`: Auxiliary function for adding three-dimensional heatmaps that to plots produced by either `regplane3D` or `twoplanes3D`. These heatmaps show the joint frequency/density distribution of the model predictors that defines the horizontal axes of the plots.
...
The use of the `regplane3D` functions requires that users provide the following inputs:

1. A vector containing values of the first predictor, $X$.
1. A vector containing values second predictor, $Y$.
1. A matrix containing the expected values of $Z$, or an array containing the expected values as well as their lower and upper bounds, for all combinations of the specified values for $X$ and $Y$.
1. *Optional*: A matrix containing the discretized density or frequency of $X$ and $Y$.
...
internal data set `us`. For a documentation of the data, run `?regplane3D::us`.
The data set is printed below:
...
predictor. Our first example uses the `regplane3D()` function to illustrate
this with the example of an OLS regression of incumbent vote shares in US
...
and save the resulting estimation object named `mod`.

```{r use-regplane3D-step1}
## ---- Estimation ----
...
to round the range of a variable to a custom base:
...
At each iteration of the loop, we can extract the expected value and calculate
the lower and upper bounds of the 95% confidence interval using 
`pred_tmp$fit + qnorm(.025) * pred_tmp$se.fit` and
`pred_tmp$fit + qnorm(.975) * pred_tmp$se.fit`, respectively, which we
subsequently store them in the appropriate cell of the array `pred`. The array
is of dimensions `dim = c(21L, 21L, 3L)`. The first dimension represents the
values of `growth_seq`, the second dimension represents the values of
`approval_seq`, and the third dimension represents the point estimates,
lower confidence bounds, and upper confidence bounds.

```{r use-regplane3D-step2}
## ---- Prediction ----
...
For additional options, see `?regplane3D`.

```{r use-regplane3D-step3}
## ---- Plot ----
par(mar = c(1.1, 2.1, 4.1, 0.1))
regplane3D(
  z = pred,
  x = growth_seq,
  y = approval_seq,
...
`heatmap = joint_frequency` to our `regplane3D()` command:

```{r use-regplane3D-step5}
## ---- Plot ----
par(mar = c(1.1, 2.1, 4.1, 0.1))
regplane3D(
  z = pred,
...
output from `plot3D` functions (suing the option `add = TRUE`).
For instance, we can add the true values of the outcome variable as points
(using `plot3D::points3D()`) and add labels using `plot3D::text3D`.

```{r use-regplane3D-step6}
## ---- Plot ----
par(mar = c(1.1, 2.1, 4.1, 0.1))
regplane3D(
  z = pred,
...
The `twoplanes3D()` function extends the functionality of `regplane3D()` to
...
expected values and confidence bounds across the 21 values `growth_range` and
11 values ranging from `min(centered_approval)` up to the cutpoint of
...
approval ratings in `pred[, , 2, ]`, where the 11 values of `centered_approval`
...
growth_seq <-
  seq(growth_range[1], growth_range[2], length.out = 11L)
approval_seq <-
  seq(approval_range[1], approval_cut_point, length.out = 11L)
...
Plotting with `twoplanes3D()` works the same way as with `regplane3D()`, except
...
Extensions can be added as with `regplane3D()`.

```{r use-twoplanes3D-step2}
## ---- Plot ----
